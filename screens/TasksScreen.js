...
const HomeScreen = () => {
  ...
  const fetchTasks = async () => {
    const response = await fetch(
      `${configuration[appContext.env].api.baseUrl}api/app_tasks/user_tasks`,
      { method: "GET", headers: { Authorization: "" + appContext.userdata.access_token, },
      }
    );
  ...
  const handleNewTask = () => {
    Alert.alert( "Enter the name of the task", '', (taskName) => { console.log(taskName); } );
  };
  return (
